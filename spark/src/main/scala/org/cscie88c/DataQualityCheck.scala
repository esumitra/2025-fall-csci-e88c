...
object DataQualityChecks {
    ...
    def nullPercentages(df: DataFrame)(implicit spark: SparkSession): DataFrame = {
        ...
        // Map over Scala sequence of column names
        val nullStats = df.columns.toSeq.map { 
            colName => val nullCount = 
                df.filter(df(colName).isNull).count()
            (colName, nullCount / total * 100)
        }

        // Convert results back into DataFrame
    ...
    trips
        .withColumn("valid_trip_distance", col("trip_distance") > 0)
        .withColumn("valid_fare_amount", col("fare_amount") > 0)
        .withColumn("pickup_before_dropoff",
