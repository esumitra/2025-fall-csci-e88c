...
object SilverJob {
  ...
  def cleanData(
      tripsParquetPath: String,
      taxiZonesCsvPath: String,
      outputExtraFiles: Boolean
    ...
    val withDerived = tripsDF
      .withColumn(
        "trip_duration_min",
        (col("tpep_dropoff_datetime").cast("long") - col("tpep_pickup_datetime")
          .cast("long")) / 60.0
      )
      .withColumn("pickup_date", to_date(col("tpep_pickup_datetime")))
      .withColumn("pickup_hour", hour(col("tpep_pickup_datetime")))

    val totalRows = withDerived.count()
    ...
    // ============================================================
    val numericCols = withBasicDQ.schema.fields
      .filter(f =>
    ...
    if (numericCols.nonEmpty) {
      ...
      numericallyCleanDF = applyDQRule(
        numericallyCleanDF,
        "no_negative_values",
        negativeCondition,
        outputExtraFiles
    ...
    println(
      f"Initial rows: $totalRows%,d | Final kept: $finalCleanCount%,d | Removed: ${totalRows - finalCleanCount}%,d"
    )
    ...
    if (outputExtraFiles) {
      ...
      println(
        f"✅ 10K-row clean sample CSV written to: $sampleCsvOutClean ($sampleCountClean%,d rows)"
      )
    }
    ...
    val conformedTripsDF = numericallyCleanDF
      .withColumn(
        "tpep_pickup_datetime_est",
        from_utc_timestamp(
          to_utc_timestamp(col("tpep_pickup_datetime"), "UTC"),
          "America/New_York"
        )
      )
      .withColumn(
        "tpep_dropoff_datetime_est",
        from_utc_timestamp(
          to_utc_timestamp(col("tpep_dropoff_datetime"), "UTC"),
          "America/New_York"
        )
      )
      ...
      .withColumn(
        "pickup_week_start",
        date_sub(next_day(col("pickup_date"), "Mon"), 7)
      )
    ...
    val joinedTripsDF = conformedTripsDF
      .join(
        cleanedZonesDF,
        conformedTripsDF("PULocationID") === cleanedZonesDF("LocationID"),
        "left"
      )
    ...
    val condition: Column = not(
      col("pickup_borough").isNull || trim(col("pickup_borough")) === ""
    )
    val finalConformedDF =
      applyDQRule(joinedTripsDF, "Invalid Zones", condition, outputExtraFiles)
    ...
    println(
      s"✅ Conformed Silver data written to: $Utilities.SilverTripsConformed"
    )
    ...
    if (outputExtraFiles) {
      ...
      println(
        f"✅ 10K-row conformed sample CSV written to: $sampleCsvOutConf ($sampleCountConf%,d rows)"
      )
    }
    // ============================================================
    // Step 11: Preview
  ...
  private def applyBasicDQRules(
      df: DataFrame,
      outputExtraFiles: Boolean
  ): DataFrame = {
    var currentDF = df
    currentDF = applyDQRule(
      currentDF,
      "Non-null",
      currentDF.columns.map(c => col(c).isNotNull).reduce(_ && _),
      outputExtraFiles
    )
    currentDF = applyDQRule(
      currentDF,
    ...
    currentDF = applyDQRule(
      currentDF,
      "duration_reasonable",
      col("trip_duration_min").between(1.0, 180.0),
      outputExtraFiles
    )
    currentDF = applyDQRule(
      currentDF,
  ...
  def applyDQRule(
    ...
    println(
      f"Total before: $totalBefore%,d | Kept: $keptCount%,d | Rejected: $rejectedCount%,d (${rejectRate * 100}%.2f%%)"
    )

    if (rejectRate > dqThreshold)
      throw new RuntimeException(
        f"❌ FAIL-FAST: Rule [$ruleName] reject rate ${rejectRate * 100}%.2f%% exceeds ${dqThreshold * 100}%.1f%% threshold."
      )
